...
void misc_init_r_env(void){
    ...
    envSetDefault("console", "console=ttyS0,9600");
       ...
       envSetDefault("mtdparts", "mtdparts=pxa3xx_nand-0:1024m(flash)");
...
#else
    fdt_env_setup("msys-modular.dtb", MV_FALSE); /* dynamic setup: run DT update (false since not supported yet) */
#endif
...
// Temporary workaround till stage_boot gets stable.
       setenv("ubiboot", "setenv initrd_high 0xffffffff;"
                  "setenv fdt_skip_update no;"
                  "ubi part flash;ubifsmount flash;ubifsload 0x5000000 /diags/Nadiag.img;"
                  "bootm 0x5000000");
