		...
		*/
		...
		if (status == MV_ERROR)
			...
			if (status == MV_OK) {
				...
				mvOsDelay(500);
				{
				printf("\t[BlueDBG] goto PCIE WA!\n");
				printf("\tbefore\n\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_CAUSE_REG(pexIf), MV_REG_READ(PEX_CAUSE_REG(pexIf)) );
				printf("\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_CTRL_REG(pexIf), MV_REG_READ(PEX_CTRL_REG(pexIf)) );
				printf("\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_STATUS_REG(pexIf), MV_REG_READ(PEX_STATUS_REG(pexIf)) );
				printf("\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_DBG_STATUS_REG(pexIf), MV_REG_READ(PEX_DBG_STATUS_REG(pexIf)) );
				    int val = MV_REG_READ(0x41a60);
				    int i=0;

				   if( ((MV_REG_READ(PEX_DBG_STATUS_REG(pexIf)) & 0x7f) != 0x7E)
					  && (MV_REG_READ(PEX_STATUS_REG(pexIf)) & PXSR_DL_DOWN) ) {
					   MV_REG_WRITE(0x41a60,val | (1 << 20)); //soft reset
					   do {
					       mvOsDelay(100);
					   }while((MV_REG_READ(PEX_STATUS_REG(pexIf)) & PXSR_DL_DOWN) && i++ < 10);
					   MV_REG_WRITE(0x41a60,val);
					   /*MV_REG_WRITE(0x00040090,0x00000002);
					   MV_REG_WRITE(0x00041A00,0x4014062E);*/
				    }
				printf("\tafter\n\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_CAUSE_REG(pexIf), MV_REG_READ(PEX_CAUSE_REG(pexIf)) );
				printf("\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_CTRL_REG(pexIf), MV_REG_READ(PEX_CTRL_REG(pexIf)) );
				printf("\t\t MV_REG_READ(0x%x) = 0x%x\n", PEX_STATUS_REG(pexIf), MV_REG_READ(PEX_STATUS_REG(pexIf)) );
