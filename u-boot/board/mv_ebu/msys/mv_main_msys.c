...
#if 1 /* WNC - Slow down FAN RPM.by default */
#include <i2c.h>
#define ADT7476_I2C_ADDR        0x2E
#define ADT7476_CFG_ADDR        0x40
#define ADT7476_PWM1_CFG_ADDR       0x5C
#define ADT7476_PWM1_FRQ_ADDR       0x5F
#define ADT7476_PWM1_CUR_ADDR       0x30
#define ADT7476_TACH1_LOW_ADDR      0x28
#define ADT7476_TACH1_HI_ADDR       0x29
#define ADT7476_TACH2_LOW_ADDR      0x2A
#define ADT7476_TACH2_HI_ADDR       0x2B
#define ADT7476_CFG_ADDR_VAL        0x5
#define ADT7476_PWM_CFG_ADDR_VAL    0xE2
#define ADT7476_PWM_FRQ_ADDR_VAL    0xCC
#define ADT7476_PWM_CUR_ADDR_VAL    0x80
#endif
...
#else
    envSetDefault("mtdparts", "mtdparts=pxa3xx_nand-0:8m(boot)ro,8m@8m(kernel),944m@16m(rootfs),-(log)");
    envSetDefault("mtdids", "nand0=pxa3xx_nand-0");
#endif /* CONFIG_WNC_AC3_SWITCH */
...
#else
    setenv("ubiboot","setenv bootargs root=ubi0 rw ubi.mtd=2 rootfstype=ubifs "
           "console=ttyS0,115200 ${othbootargs} ${mtdparts_linux};"
           "ubi part rootfs;ubifsmount nand_rootfs;ubifsload 0x2000000 /boot/zImage;"
           "ubifsload 0x1000000 /boot/msys-ac3-rd.dtb;bootz 0x2000000 - 0x1000000");
...
#if 0 /* WNC - Slow down FAN RPM. */
{
    uint8_t data;
    int ret;

        /* Set start bit to enable the chip. */
        data = ADT7476_CFG_ADDR_VAL;
        if ((ret = i2c_write(ADT7476_I2C_ADDR, ADT7476_CFG_ADDR, 1, &data, 1)) != 0) {
                printf("Got failed to set start bit with ret 0x%x!\n", ret);
                goto skip;
        }

        /* Enable high frequency mode. */
        data = ADT7476_PWM_FRQ_ADDR_VAL;
        if ((ret = i2c_write(ADT7476_I2C_ADDR, ADT7476_PWM1_FRQ_ADDR, 1, &data, 1)) != 0) {
                printf("Got failed to enable high frequency mode for FAN1 with ret 0x%x!\n", ret);
                goto skip;
        }

    /* Set manual mode. */
        data = ADT7476_PWM_CFG_ADDR_VAL;
        if ((ret = i2c_write(ADT7476_I2C_ADDR, ADT7476_PWM1_CFG_ADDR, 1, &data, 1)) != 0) {
                printf("Got failed to set manual mode for FAN1 with ret 0x%x!\n", ret);
                goto skip;
        }

        /* Set RPM. */
        data = ADT7476_PWM_CUR_ADDR_VAL;
        if ((ret = i2c_write(ADT7476_I2C_ADDR, ADT7476_PWM1_CUR_ADDR, 1, &data, 1)) != 0) {
                printf("Got failed to set RPM for FAN1 with ret 0x%x!\n", ret);
                goto skip;
        }

skip:
        if (!ret)
                printf("Slow down FAN RPM.\n");
