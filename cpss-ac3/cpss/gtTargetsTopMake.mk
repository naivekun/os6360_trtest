...
ifeq (MP_ON,$(CPSS_USE_MUTEX_PROFILER))
	CFLAGS += $(FD)CPSS_USE_MUTEX_PROFILER
endif

ifeq (TRAFFIC_API_LOCK_DISABLE,$(CPSS_TRAFFIC_API_LOCK))
	CFLAGS += $(FD)CPSS_TRAFFIC_API_LOCK_DISABLE
endif
...
ifneq (,$(DEP_FILES))
  ...
  ifeq (, $(filter _$(MAKECMDGOALS),_dep _clean_p _mkld _modules))
    include $(DEP_FILES)
  endif
endif
...
ifeq (yes,$(DEPENDENCIES_COMPLETE))
dep:
else
...
compile_nr: $(OBJ_FILE_LIST) $(ASM_OBJ_FILE_LIST)
	+@$(QUIET_CALL)set -e;$(DO_KERNELLOOP)
...
ifneq (0,$(words $(EXE_OBJ_LIST)))
  ifeq (visual, $(TOOL))
    ifndef ROL_WINE_ENV
		...
		$(AR) /nologo /out:$(LIB_DIR)/$(ARCH_NAME).$(LIBEXT) $(patsubst /%.obj, Z:/%.obj, $(SCRAP_DIR)/*.$(OBJ)) /IGNORE:4006 
    endif
  else
    ifeq (builder, $(TOOL))
...
ifdef ROL_WINE_ENV
	...
	$(ECHO) MD $@
	$(MD)    $(MDFLAGS) $(subst /I ,-I,$(INCLUDE_PATH) $(PROJECT_PATH)) -MM -o $(SCRAP_DIR)/$(notdir $@) $(filter %/$(notdir $<),$(C_FILE_LIST_AFTER_EXCLUDE))
	sed -i -e 's#$(CPSS_PATH)/##g' $(SCRAP_DIR)/$(notdir $@)
else
	$(MD) -f- $(MDFLAGS) $(subst /I ,-I,$(INCLUDE_PATH) $(PROJECT_PATH)) $(filter %/$(notdir $<),$(C_FILE_LIST_AFTER_EXCLUDE)) > $(SCRAP_DIR)/$@ 
endif
endif
