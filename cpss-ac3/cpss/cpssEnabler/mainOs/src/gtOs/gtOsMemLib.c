...
/* total number of bytes allocated for the DMA */
static GT_U32   dmaTotalAlloc __SHARED_DATA_MAINOS = 0;
/* allow trace alloc of DMA */
static GT_U32   dmaTotalAlloc_traceEnable __SHARED_DATA_MAINOS = 0;
/* allow to 'see' the total DMA allocations */
void   prvOsDmaTotalAllocPrint(void)
{
    static GT_U32   lastValue = 0;
    ...
    lastValue = dmaTotalAlloc;
}
/* allow to 'see' the total DMA allocations */
void   prvOsDmaTotalAllocTraceEnable(IN GT_U32  enable)
{
    dmaTotalAlloc_traceEnable = enable;
}
    ...
    *outSizePtr = size;
    ...
    if (allocVarPtr == NULL)
    {
        ...
        /* and as a precheck that the next element can or cannot be allocated from
           this chunk */
        while (poolHdrPtr->chunkIndex < os_mem_chunks_array.lastChunkIndex &&
            os_mem_chunks_array.chunkArray[poolHdrPtr->chunkIndex].freeSize <= os_mem_poolsLimit)
        {
            poolHdrPtr->chunkIndex++;
        }
...
)
{
    ...
    if(rc == GT_OK)
    {
        dmaTotalAlloc += numOfBytes;
        if(dmaTotalAlloc_traceEnable)
        {
...
)
{
    ...
    dmaTotalAlloc -= elementPtr->myMemoryNumOfBytes;
    if(dmaTotalAlloc_traceEnable)
    {
        prvOsDmaTotalAllocPrint();
    }
...
static GT_VOID dmaFreeAll(GT_VOID)
{
    ...
    globalDmaAllocations.myMagic = MAGIC_NUMBER_CNS;
