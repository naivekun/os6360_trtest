...
/* taskLock data */
static pthread_mutex_t taskLock_mtx __SHARED_DATA_MAINOS;
static pthread_cond_t taskLock_cond __SHARED_DATA_MAINOS;
static int taskLock_count __SHARED_DATA_MAINOS;
static pthread_t taskLock_owner __SHARED_DATA_MAINOS;
#endif
...
)
{
    ...
    if (!initialized)
    {
        CREATE_MTX(&tasks_mtx);
        CREATE_MTX(&taskLock_mtx);
        CREATE_COND(&taskLock_cond);
        initialized = 1;
    }
#endif
...
static int V2L_taskIdSelf_add_new_threads(pthread_t pthrid, int task_allocated)
{
    ...
    for (t = task_allocated; t < tasks.allocated; t++)
...
static int V2L_taskIdSelf(GT_VOID)
{
    ...
    for (t = 1; t < tasks.allocated; t++)
        if (TASK(t) && TASK(t)->header.type && TASK(t)->pthrid == pthrid)
            return t;
    /* gain - add thread to task list - start */
    t = V2L_taskIdSelf_add_new_threads(pthrid, tasks.allocated);
    /* gain - add thread to task list - End */
...
GT_STATUS CPSS_osTaskLock(GT_VOID)
{
    ...
    DO_LOCK_MUTEX(&taskLock_mtx);
    while (taskLock_count)
    {
        if (taskLock_owner == self)
            break;
        DO_COND_WAIT(&taskLock_cond, &taskLock_mtx);
    }
    taskLock_count++;
    taskLock_owner = self;
