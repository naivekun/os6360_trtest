...
)
{
    ...
    if (pDev->haAccessDisable == MCD_FALSE)
    {
        status = (MCD_16)mcdWrapperSetTxPRBS(pSerdesDev, phySerdesNum + 1, txDataType);
    }
    mcdSerdesAccessUnlock(pDev);
    if (status != MCD_OK)
    {
...
)
{
    ...
    if (pDev->haAccessDisable == MCD_FALSE)
    {
        status = (MCD_16)mcdWrapperSetRxCmpData(pSerdesDev, phySerdesNum+1, RxCmpType);
    }
    mcdSerdesAccessUnlock(pDev);
    if (status != MCD_OK)
    {
    ...
    case MCD_CLEAR_SERDES_LOOPBACK:
        ...
        else if (pDev->serdesStatus.serdesLpbkMode[serdesNum & 7] == MCD_SHALLOW_SERDES_LOOPBACK)
        {
            if (retimerMode)
            {
                ...
                ATTEMPT(mcdHwSetPhyRegField32bit(pDev,DONT_CARE,MCD_SERDES_EXTERNAL_CONFIGURATION_1(serdesNum), 3, 1, 0x0));
                /* switch clock to local serdes */
                ATTEMPT(mcdSerdesSpicoInterrupt(pDev, serdesNum, 0x30,0x70, NULL));
                ...
                ATTEMPT(mcdHwSetPhyRegField32bit(pDev,DONT_CARE,MCD_SERDES_EXTERNAL_CONFIGURATION_1(serdesNum), 3, 1, 0x1));
            }
            else
            {
