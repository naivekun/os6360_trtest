...
{
   if(((( portMode ==  MCD_MODE_P100L  ) || ( portMode ==  MCD_MODE_P40L   ) || ( portMode ==  MCD_MODE_P50R2L ) ||
       ( portMode ==  MCD_MODE_P40R2L ) || ( portMode ==  MCD_MODE_P25L    ) || ( portMode ==  MCD_MODE_P10L   ) /*||
       ( portMode ==  MCD_MODE_G21L   )*/ ) && (fecCorrect != MCD_NO_FEC)) ||
       (((portMode == MCD_MODE_P10S) ||(portMode == MCD_MODE_P40C) || (portMode == MCD_MODE_P40K) || (portMode == MCD_MODE_P40S)||
         (portMode == MCD_MODE_G21SK) || ( portMode ==  MCD_MODE_G21L)) &&
       (fecCorrect & (MCD_RS_FEC_HOST | MCD_RS_FEC_LINE | MCD_RS_FEC))) ||
       (((portMode == MCD_MODE_P100S) || (portMode == MCD_MODE_P100C) || (portMode == MCD_MODE_P100K)) &&
       (fecCorrect & (MCD_FC_FEC_HOST | MCD_FC_FEC_LINE | MCD_FC_FEC))))
       {
...
)
{
    ...
    /* Fall back Link timer to default value */
        ...
        case MCD_MODE_P10S:
        {
            ...
            if ((chan == 0) || (chan == 1))
            {
                ...
                else
                {
                    ...
                    /*ATTEMPT(mcdHwSetPhyRegField32bit(pDev, mdioPort, UMAC_GLBL_Global_Mode(slice), 0, 4, UMAC_GLOBAL_MODE_4x25G));*/
                }
            }
            else /* ((chan == 2) || (chan == 3)) */
            {
...
)
{
    ...
    if (!pDev->devEnabled)
    {
        MCD_DBG_ERROR("mcdPortSerdesTxEnable: device not initialized\n");
        return MCD_FAIL;
    }
    ...
    switch(portMode)
    {
        ...
        case MCD_MODE_G21SK:
            ...
            {
                MCD_DBG_ERROR("mcdPortSerdesTxEnable: wrong channel number %u\n", chan);
                return MCD_FAIL;
            }
        ...
        case MCD_MODE_R1C:
            ...
            break;
        default:
            MCD_DBG_ERROR("mcdPortSerdesTxEnable: incorrect mode selection: %u\n", portMode);
            return MCD_FAIL;
    }
...
)
{
   ...
   if (anEnable)
   {
       ATTEMPT(mcdHwSetPhyRegField32bit(pDev, mdioPort, UMAC_LSMCPCS_lnktimer(slice, channelUnit), 0, 16, 0x140));
       ATTEMPT(mcdHwSetPhyRegField32bit(pDev, mdioPort, UMAC_LSMCPCS_lnktimer2(slice, channelUnit), 0, 8, 0x82));
   }
   else
   {
       /* fall back linktimer to default */
       ATTEMPT(mcdHwSetPhyRegField32bit(pDev, mdioPort, UMAC_LSMCPCS_lnktimer(slice, channelUnit), 0, 16, 0x12D0));
       ATTEMPT(mcdHwSetPhyRegField32bit(pDev, mdioPort, UMAC_LSMCPCS_lnktimer2(slice, channelUnit), 0, 8, 0x13));
   }
   /*  Set LS speed to 1G and enable AN */
   cntlData = 0x800 + anEnable; /* 0x800 - 1G speed*/
   ATTEMPT(mcdHwXmdioWrite32bit(pDev, mdioPort, UMAC_LSMCPCS_ctrl(slice, channelUnit), cntlData));
...
)
{
   ...
   /* Set Channel mode to 10G/25G*/
    ...
    ATTEMPT(mcdConfigurePcsCommon(pDev, slice, channelUnit, fecCorrect, 3, 1));
   return MCD_OK;
}
    ...
    switch(portMode)
    {
        ...
        case MCD_MODE_P25S:
            if (host_or_line == MCD_HOST_SIDE)
            {
                ...
                {
                    ATTEMPT(mcdSerdesRxPowerCtrl(pDev, MCD_SERDES_NUM((MCD_HOST_SIDE_START_SERDES_NUMBER + chan), slice), MCD_TRUE, MCD_25_78125G, noPpmEnable,(MCD_U32)refClkSel,(MCD_U32)refClkDiv));
                    ATTEMPT(mcdSerdesTxPowerCtrl(pDev, MCD_SERDES_NUM((MCD_HOST_SIDE_START_SERDES_NUMBER + chan), slice), MCD_TRUE, MCD_25_78125G, noPpmEnable,(MCD_U32)refClkSel,(MCD_U32)refClkDiv));
                }
                else
                {
                    ATTEMPT(mcdSerdesPowerCtrl(pDev, MCD_SERDES_NUM((MCD_HOST_SIDE_START_SERDES_NUMBER + chan), slice), MCD_TRUE, MCD_25_78125G, noPpmEnable, (MCD_U32)refClkSel, (MCD_U32)refClkDiv));
                }
            ...
            {
                ATTEMPT(mcdSerdesPowerCtrl(pDev, MCD_SERDES_NUM(chan, slice), MCD_TRUE, MCD_25_78125G, MCD_FALSE, (MCD_U32)refClkSel, (MCD_U32)refClkDiv));
...
)
{
    ...
    switch (portMode)
    {
        ...
        for (laneNum=0; laneNum < numOfActiveLanes; laneNum++)
        {
            if (host_or_line == MCD_HOST_SIDE)
            {
                serdesArr[laneNum] =  MCD_SERDES_NUM((MCD_HOST_SIDE_START_SERDES_NUMBER + chan + laneNum), slice);
            }
            else /* MCD_LINE_SIDE */
            {
                serdesArr[laneNum] =  MCD_SERDES_NUM((chan + laneNum), slice);
            }
        }
...
)
{
    ...
    if (mcdIsPortValid(pDev, mdioPort) != MCD_OK)
    {
        MCD_DBG_ERROR("mcdPortPollingParamsGet: incorrect port selection: %u\n", mdioPort);
        return MCD_FAIL;
    }
    slice = mdioPort / MCD_MAX_CHANNEL_NUM_PER_SLICE;
    chan = CHAN_IN_SLICE_NUM(mdioPort);
    ...
    switch(pDev->portConfig[mdioPort].portMode)
    {
        ...
        default:
            MCD_DBG_ERROR(
                "mcdPortPollingParamsGet: incorrect mode selection: %u\n",
                pDev->portConfig[mdioPort].portMode);
            return MCD_FAIL;
    }
