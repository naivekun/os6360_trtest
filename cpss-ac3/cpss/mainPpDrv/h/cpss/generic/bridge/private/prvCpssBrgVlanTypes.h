...
/**
* @struct PRV_CPSS_ANALYZER_PORT_TAG_INFO_STC
 *
 * @brief A structure to hold analyzer port info
*/
typedef struct{

    /** analyzer port device number; */
    GT_U8 analyzerDevNum;

    /** analyzer port port number */
    GT_U8 analyzerPortNum;

    /** determine if the port is tagged or not */
    GT_BOOL isTagged;
