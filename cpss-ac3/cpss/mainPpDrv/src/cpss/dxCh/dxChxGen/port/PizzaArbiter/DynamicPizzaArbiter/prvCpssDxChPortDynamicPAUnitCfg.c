...
)
{
    ...
    {
        if (ptr->speed == INVALID_SPEED_CNS)
        {
            break;
        }
    }
...
)
{
    ...
    SortPortsInDecreasingOrder(portListPtr->portsConfigArray, CPSS_MAX_PORTS_NUM_CNS);

    portListPtr->numberOfPorts = CalculateNumberOfPorts(portListPtr);
