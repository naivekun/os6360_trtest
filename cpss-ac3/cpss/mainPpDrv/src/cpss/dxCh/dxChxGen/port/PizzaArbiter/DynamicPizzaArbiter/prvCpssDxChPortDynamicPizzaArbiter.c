...
typedef struct
{
    ...
    GT_BOOL         prv_specialTxQClientArr                                   [PRV_CPSS_MAX_PP_PORTS_NUM_CNS];
...
(
    ...
    IN  GT_U8                              devNum,
    IN  GT_BOOL                            specialTxQClientArr[PRV_CPSS_MAX_PP_PORTS_NUM_CNS],
...
)
{
    ...
    /* pass 1 : build bitmap of special TXQ clients */
    for (i = 0;  i < PRV_CPSS_MAX_PP_PORTS_NUM_CNS; i++)
    {
        specialTxQClientArr[i] = GT_FALSE;
    }
    for (i = 0; i < specialTxQClientsGroupListLen; i++)
    {
        ...
        if (groupPtr->isExtendedCascadeExist == GT_TRUE && groupPtr->groupSpeedMbps > 0)
        {
            ...
            {
                specialTxQClientArr[groupPtr->clientIdList[clientIdx].txqPort] = GT_TRUE;
            }
        }
    }
    /* pass 1 : find empty clients  */
    clientIdx = 0;
    for (i = 0; i < specialTxQClientsGroupListLen; i++)
    {
        rc = prvCpssDxChPortDynamicPATxQClientGroupListByIdxGet(devNum,i,/*OUT*/&groupPtr);
        ...
        if (groupPtr->isExtendedCascadeExist == GT_TRUE && groupPtr->groupSpeedMbps > 0)
        {
            ...
            for (; clientIdx < PRV_CPSS_MAX_PP_PORTS_NUM_CNS; clientIdx++)
            {
                if (clientSpeedConfigArr[CPSS_DXCH_PA_UNIT_TXQ_0_E][clientIdx] == 0 ) /* empty client ?*/
                {
                    if (specialTxQClientArr[clientIdx] == GT_FALSE) /* it is not appear in list of special TXQ clients ? */
                    {
                        break;  /* found */
                    }
                }
            ...
            groupPtr->tmp_groupTxqClient = clientIdx;
            clientSpeedConfigArr[CPSS_DXCH_PA_UNIT_TXQ_0_E][clientIdx] = groupPtr->groupSpeedMbps;
        }
    }
    return GT_OK;
}
    ...
    for (groupIdx = 0; groupIdx < specialTxQClientsGroupListLen; groupIdx++)
    {
        ...
        if (groupPtr->isExtendedCascadeExist == GT_TRUE && groupPtr->groupSpeedMbps > 0)
        {
            ...
            for (pizzaSliceIdx = 0 ; pizzaSliceIdx < sliceNum2Config; pizzaSliceIdx++)
            {
                ...
                while (sliceIdx < paListPtr->prv_sliceNum2Config[CPSS_DXCH_PA_UNIT_TXQ_0_E])
                {
                    if (paListPtr->prv_pizzaArray[CPSS_DXCH_PA_UNIT_TXQ_0_E][sliceIdx] == groupPtr->tmp_groupTxqClient)
                    {
                        break;
                    }
                    sliceIdx++;
                }
                if (sliceIdx == paListPtr->prv_sliceNum2Config[CPSS_DXCH_PA_UNIT_TXQ_0_E])
                {
                ...
                paListPtr->prv_pizzaArray[CPSS_DXCH_PA_UNIT_TXQ_0_E][sliceIdx] = groupPtr->clientIdList[txqClientIdx].txqPort;
                sliceIdx++;
            }
        }
    }
...
/* flag to allow optimization of the amount of times that 'PIZZA' configured
   ...
   while 'optimizing'
*/
static GT_BOOL  prvCpssDxChPortDynamicPizzaArbiterIfConfigure_optimize = GT_FALSE;
...
)
{
    ...
    if(prvCpssDxChPortDynamicPizzaArbiterIfConfigure_optimize != GT_FALSE)
    {
...
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
* @note   NOT APPLICABLE DEVICES:  DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3; Lion; Lion2; Lion2; xCat2.
*
