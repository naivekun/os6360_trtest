...
#define PRV_CPSS_HW_IN_LOGICAL_MAX_CNS 31
/* logical table entry */
typedef struct
{
...
GT_U32  debugMode_forceNoUpdateOfShadow = 0;
/* function to set <debugMode_forceNoUpdateOfShadow> */
void debugMode_cpssDxChDiagDataIntegrity_forceNoUpdateOfShadow
(
    IN GT_U32  forceNoUpdateOfShadow
)
{
    debugMode_forceNoUpdateOfShadow = forceNoUpdateOfShadow;
}
...
/*  removed because not memory in the device .. it is 'registers'
    ...
    { CPSS_DXCH_LOGICAL_TABLE_LPM_MEM_E, {  CPSS_DXCH_SIP5_TABLE_LPM_MEM_E,
        ...
        LAST_VALID_TABLE_CNS } },
    { CPSS_DXCH_LOGICAL_TABLE_ECMP_E, {  CPSS_DXCH_SIP5_TABLE_ECMP_E,
        LAST_VALID_TABLE_CNS } },
...
)
{
    ...
    for(ii = (startPtr ? (*startPtr >> 16) : 0), alterIx = (startPtr ? (*startPtr & 0xffff) : 0);
        cpssLogicalToHwTablesDB[ii].logicalName != CPSS_DXCH_LOGICAL_TABLE_LAST_E;
        ii++)
    {
        /* substitute entry from alternative DB if necessary */
        if (alterDb &&
            alterDb[alterIx].logicalName == cpssLogicalToHwTablesDB[ii].logicalName)
        {
        ...
        {
            hwNameArr = cpssLogicalToHwTablesDB[ii].hwNameArr;
        }
...
{
    GT_U32  ii;
    const PRV_CPSS_DXCH_LOGICAL_ENTRY_STC *logicalToHwDb = cpssLogicalToHwTablesDB;
...
)
{
    ...
    if((GT_U32)hwTableType >=  PRV_CPSS_DXCH_TABLE_SIZE_GET_MAC(devNum))
    {
        /* the HW table is not valid for this device */
        return GT_FALSE;
    }

    tableInfoPtr = PRV_TABLE_INFO_PTR_GET_MAC(devNum,hwTableType);
...
)
{
    ...
    if(debugMode_forceNoUpdateOfShadow)
    {
        /* !!! debug mode that force us to not update the shadow !!! */
        return GT_OK;
    }
    ...
    {
        /* the entry occupies two non-continues lines - half entry per line.
           Need to write them separately */

        if ((numEntriesPerLine - FRACTION_HALF_TABLE_INDICATION_CNS) != 2)
            /* maybe some additional logic will be required for this cases */
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
        }

         /* In case of HALF_TABLE_INDICATION numBitsPerEntry is half of
          * actual entry length - entry's part stored in single line,
          * non in both lines */

        /* copy first half of the entry. */
        ...
        /* get the shadow line storing the second half of entry */

        startBitIndex = numBitsPerEntry; /* half of entry's length */
        ...
        copyBits(shadowPtr,     /* target */
                 startBitIndex, /* start bit in the target */
                 entryValuePtr, /* source */
                 numBitsPerEntry,  /* start bit in the source */
                 numBitsPerEntry); /* number of bits to copy */
    }
    else
    {
...
)
{
    ...
    prvCpssDxChIndexAsPortNumConvert(devNum, hwTableType, hwTableEntryIndex,
                                     &unused, &hwTableEntryIndex, &hwTableType);

    shadowInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->shadowInfoArr[hwTableType]);
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        ...
        startBitIndex = 0;
        rc = prvCpssDxChPortGroupShadowEntryGet(
            devNum, portGroupId, hwTableType,
            PRV_CPSS_DXCH_SHADOW_INDEX_TYPE_ENTRY_E,
        ...
        else
        {
            ...
            else
            {
                ...
                /* get the shadow line storing the second half of entry */
                startBitIndex = numBitsPerEntry;
                rc = prvCpssDxChPortGroupShadowEntryGet(
                    devNum, portGroupId, hwTableType,
                    PRV_CPSS_DXCH_SHADOW_INDEX_TYPE_ENTRY_E,
                    hwTableEntryIndex,    /* index is of 'entry'  */
                    &shadowPtr,           /* pointer is to 'line' */
                    &startBitIndex,/* offset of the start of the 'entry' in the 'line' */
                    NULL,  /* not interested. We've got it before */
                    NULL); /* not interested. We've got it before */
                ...
                copyBits(valueArr,      /* target */
                         numBitsPerEntry,  /* start bit in target */
                         shadowPtr,     /* source*/
            ...
            resetBitsInMemory(valueArr, 2*numBitsPerEntry, 32);
...
)
{
    ...
    GT_U32  startBitIndex;/* the start bit index of entry in the line */
    GT_U32  numBitsPerEntry;/* number of bits per entry */
    INOUT GT_PORT_GROUPS_BMP portGroupsBmp;
    PRV_CPSS_DXCH_HW_TABLES_SHADOW_STC *shadowInfoPtr;
    GT_U32 numEntriesPerLine;
    ...
    if (numEntriesPerLine & FRACTION_HALF_TABLE_INDICATION_CNS)
    {
        ...
        copyBits(&entryValuePtr[0], numBitsPerEntry, /* source base, offset */
                 shadowPtr, startBitIndex,           /* target base, offset */
