...
*
* @note   APPLICABLE DEVICES:      xCat; xCat3; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
* @note   NOT APPLICABLE DEVICES:  DxCh1; DxCh1_Diamond; DxCh2; DxCh3; Lion; xCat2; Lion2.
*
* @param[in] devNum                   - device number
*
...
*
* @param[in] devNum                   - device number
* @param[in] index                    - counter number. (APPLICABLE DEVICES Bobcat2; Caelum; Bobcat3; Aldrin2; Falcon; Aldrin; AC3X)(APPLICABLE RANGES: 0..2)
*
...
)
{
    GT_U32      regAddr;
    GT_STATUS   rc;
    GT_U32    sip6_multiTarget_minValue,sip6_multiTarget_maxValue;/* multi-target : global eport : min and max values for sip 6 */
    /* check that can convert mask to range */
    rc = prvCpssDxChSip6GlobalEportMaskCheck(devNum,mask,
        value,
        &sip6_multiTarget_minValue,
        &sip6_multiTarget_maxValue);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, 17, sip6_multiTarget_minValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    return prvCpssHwPpSetRegField(devNum, regAddr, 0, 17, sip6_multiTarget_maxValue);
}
...
)
{
    ...
    regAddr = PRV_DXCH_REG1_UNIT_MLL_MAC(devNum).multiTargetEPortMap.multiTargetGlobalEPortMinValue;
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 17, &sip6_multiTarget_minValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    regAddr = PRV_DXCH_REG1_UNIT_MLL_MAC(devNum).multiTargetEPortMap.multiTargetGlobalEPortMaxValue;
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 17, &sip6_multiTarget_maxValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    /* check that can convert mask to range */
    rc = prvCpssDxChSip6GlobalEportMaskGet(sip6_multiTarget_minValue,
                                 sip6_multiTarget_maxValue,
                                 valuePtr,
                                 maskPtr);
