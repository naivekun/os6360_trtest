...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        {
            rc = sip6FdbAuMsgSpecialMuxedFieldsGet_mac(devNum,hwDataArr,specialFieldsPtr);
            if(rc != GT_OK)
            {
                return rc;
            }
    ...
    switch (macEntryKeyPtr->entryType)
    {
        ...
        case CPSS_MAC_ENTRY_EXT_TYPE_IPV6_MCAST_E:
            ...
            if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
            {
                ...
                {
                    if((GT_U32)(vlanId & 1) != ((hwValue >> 26) & 1))
                    {
                        /* JIRA : MT-358 - in the MC-IP update message : Bit 77 is part of 2 fields (non-muxed fields) : fid and sip[26:0] */
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                            "Erratum : Bit 0 of macEntryKeyPtr->key.ipMcast.vlanId[0x%4.4x] must be with same value as bit 26 in macEntryKeyPtr->key.ipMcast.sip[0x%8.8x]",
                            vlanId ,
                            hwValue);
                    }
                }
    ...
    if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        switch(macEntryPtr->key.entryType)
        {
            ...
            case CPSS_MAC_ENTRY_EXT_TYPE_IPV6_UC_ADDR_ENTRY_E:
                ...
                for (i = 0; i < 4; i++)
                {
                    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.FdbIpLookup.FdbIpv6RouteLookupMask[0];
                ...
                /* Set IPv6 address pattern */
                for (i = 0; i < 16; i++)
                {
                    ipv6LookupPattern[i >> 2] += macEntryPtr->key.key.ipv6Unicast.dip[i] << ((3-(i & 3))*8);
                }
