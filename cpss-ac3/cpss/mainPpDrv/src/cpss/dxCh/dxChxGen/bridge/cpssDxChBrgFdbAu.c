...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
    {
        (void) prvCpssDxChBrgFdbAuMsgSpecialMuxedFieldsGet(devNum,auMsgArrayOfWordsPtr,macEntryType,&specialFields);
        ...
        if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
        {
            ...
            if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    ...
                    hwValue);
                SIP6_CONVERT_HW_DA_CMD_TO_SW_VAL_MAC(devNum,hwValue,addrUpMsgPtr->macEntry.daCommand);

                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    SIP6_FDB_AU_MSG_TABLE_FIELDS_SA_CMD_E,
                    hwValue);
                SIP6_CONVERT_HW_SA_CMD_TO_SW_VAL_MAC(devNum,hwValue,addrUpMsgPtr->macEntry.saCommand);
...
*                                      were received
*
* @param[out] auFuMessagesPtr          - array that holds received AU / FUmessages
*                                      pointer is allocated by the caller , with enough memory
*                                      for numOfAuPtr entries that was given as INPUT
*                                      parameter.
*
