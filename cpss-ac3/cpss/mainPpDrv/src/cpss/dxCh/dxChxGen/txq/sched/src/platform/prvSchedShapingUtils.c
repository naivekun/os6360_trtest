...
#define NODE_SHAPER_ACCURACY            0.02

#define MINIMAL_PORT_SHAPER_ACCURACY    0.005
#define OPTIMAL_PORT_SHAPER_ACCURACY    0.0001


int prvSchedShapingUtilsCalculateNodeShapingNoBurstSizeAdaption(    double      bw_in_tokens,
                            ...
                            )
{
    ...
    *presolution=0;
    ...
    while( divider_exp > 0 )
    {
        ...
        if (schedFabsDb(accuracy) > NODE_SHAPER_ACCURACY) break;
        bw_in_tokens = next_bw_in_tokens;
        divider_exp--;
    }
    /* check if */
    token=(uint16_t )(bw_in_tokens + 0.5);
    /* check if it enough accurate*/
    accuracy=(1.0*token-bw_in_tokens)/bw_in_tokens;
    if (schedFabsDb(accuracy) > NODE_SHAPER_ACCURACY) return 3;
...
int prvSchedShapingUtilsCalculateNodeShaping( double        cir_units,                  
                            double      eir_units,                  
                            uint16_t *  pcbs_kb,                        
                            uint16_t *  pebs_kb,                        
                            uint32_t    token_resolution_multiplyer,
                            ...
                            )
{
    ...
    switch (prvSchedShapingUtilsCalculateNodeShapingNoBurstSizeAdaption(cir_units, 
                                                &burst, 
        ...
        case 2:
            {
                *pcbs_kb =(uint16_t)((burst+512) /1024);
                burst = *pebs_kb * 1024;
                prvSchedShapingUtilsCalculateNodeShapingNoBurstSizeAdaption(eir_units, &burst, token_resolution_multiplyer, peir_divider_exp, peir_resolution, peir_token,hndl,out_accuracy_ptr);
    ...
    burst = *pebs_kb * 1024;
    switch (prvSchedShapingUtilsCalculateNodeShapingNoBurstSizeAdaption(eir_units, &burst, token_resolution_multiplyer, peir_divider_exp, peir_resolution, peir_token,hndl,NULL))
    {
        case 1: return TM_BW_OUT_OF_RANGE;
        case 2:
            {
                        ...
                        )
{
    ...
    burst = *pcbs_kb * 1024;
    result_1 = prvSchedShapingUtilsCalculateNodeShapingNoBurstSizeAdaption(cir_units, &burst, token_resolution_multiplyer, &divider_exp, &resolution,&token,NULL,&out_accuracy);
    if (result_1 == 2) 
    {
    ...
    burst = *pebs_kb * 1024;
    result_2 = prvSchedShapingUtilsCalculateNodeShapingNoBurstSizeAdaption(eir_units, &burst, token_resolution_multiplyer, &divider_exp, &resolution,&token,NULL,&out_accuracy);
    if (result_2 == 2) 
    {
