...
)
{
    ...
    if(CPSS_IS_PX_FAMILY_MAC(PRV_CPSS_PP_MAC(devNum)->devFamily))
    {
        ...
        rc = prvCpssPxHwRegAddrStcInfoGet(devNum, dbKind, regAddrPtrPtr, sizePtr);
...
)
{
    ...
    rc = printPpDumpTables(devNum,
                            (prvCpssDrvPpConfig[devNum]->portGroupsInfo.isMultiPortGroupDevice == GT_FALSE) ? CPSS_PORT_GROUP_UNAWARE_MODE_CNS : portGroupId,
                            regArray, size);
    return rc;
}
