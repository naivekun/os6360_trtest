...
GT_STATUS mvHwsComHRev2IfInit(MV_HWS_SERDES_FUNC_PTRS *funcPtrArray)
{
  ...
  funcPtrArray[COM_PHY_H_REV2].serdesSeqSetFunc         = NULL;
#endif
...
)
{
  ...
  {
      /* Puma3B0 addition */
      CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_GEN1_SET1_Reg, (txAmpShft << 15), (1 << 15)));
  }
  else
  {
      /* Lion2B addition */
      CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_TX_AMP_Reg, txAmpShft, (1 << 0)));
  }

  data = (txAmpAdj ? 1:0);
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_TX_AMP_Reg, (data << 6), (1 << 6)));
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_TX_AMP_Reg, (txAmp << 1), 0x3E));

  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_GEN1_SET2_Reg, emph0, 0xF));
  data = (txEmphEn ? 1:0);
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_GEN1_SET2_Reg, (data << 4), (1 << 4)));
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_TX_AMP_Reg, (emph1 << 7), 0x780));
...
)
{
  ...
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_RX_FFE_Reg, (ffeRes << 4), (7 << 4)));
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_RX_FFE_Reg, ffeCap, 0xF));
  ...
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_OS_PH_OFFSET_Reg, alig, 0x007F));
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_OS_PH_OFFSET_Reg, 1<<8, 1<<8));
  CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, baseAddr+COM_H_REV2_OS_PH_OFFSET_Reg, 0, 1<<8));
...
)
{
    ...
    CHECK_STATUS(mvHwsComHRev2SerdesManualTxConfig(devNum, portGroup, serdesNum,
                                                   ...
                                                   U32_GET_FIELD(tx3Data, 7,  1), GT_TRUE)); /* Tx Amp Shft */
