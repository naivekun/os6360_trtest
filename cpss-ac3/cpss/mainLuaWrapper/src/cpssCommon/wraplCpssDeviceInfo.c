...
int luaCLI_getPortGroups(lua_State *L)
{
    ...
    for((portGroupIndex) = prvCpssDrvPpConfig[devNum]->portGroupsInfo.firstActivePortGroup;
           (portGroupIndex) <= prvCpssDrvPpConfig[devNum]->portGroupsInfo.lastActivePortGroup; portGroupIndex++)
    {
        if(0 != (prvCpssDrvPpConfig[devNum]->portGroupsInfo.activePortGroupsBmp & (1<<(portGroupIndex))))
        {
