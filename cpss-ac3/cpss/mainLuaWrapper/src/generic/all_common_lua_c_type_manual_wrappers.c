...
)
{
    ...
    switch (key->entryType)
    {
        ...
        case CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_FID_VID1_E:
            /* macVlan */
            lua_newtable(L);
            FO_STRUCT(&(key->key.macVlan), t+2, macAddr, GT_ETHERADDR);
            FO_NUMBER(&(key->key.macVlan), t+2, vlanId, GT_U16);
            lua_setfield(L, t+1, "macVlan");
            break;
        ...
        case CPSS_MAC_ENTRY_EXT_TYPE_IPV4_MCAST_FID_VID1_E:
            ...
            lua_setfield(L, t+2, "sip");
            prv_c_to_lua_GT_IPV4(L, key->key.ipMcast.dip);
            lua_setfield(L, t+2, "dip");
            FO_NUMBER(&(key->key.ipMcast), t+2, vlanId, GT_U16);
            lua_setfield(L, t+1, "ipMcast");
            break;
        ...
        case CPSS_MAC_ENTRY_EXT_TYPE_IPV6_MCAST_FID_VID1_E:
            /* ipMcast */
            lua_newtable(L);
            FO_ARRAY_START(&(key->key.ipMcast), t+2, sip);
            {
                int idx;
                for (idx = 0; idx < 4; idx++) {
            ...
            FO_ARRAY_END(&(key->key.ipMcast), t+2, sip);
            FO_ARRAY_START(&(key->key.ipMcast), t+2, dip);
            {
                int idx;
                for (idx = 0; idx < 4; idx++) {
            ...
            FO_ARRAY_END(&(key->key.ipMcast), t+2, dip);
            FO_NUMBER(&(key->key.ipMcast), t+2, vlanId, GT_U16);
            lua_setfield(L, t+1, "ipMcast");
            break;
        ...
        case CPSS_MAC_ENTRY_EXT_TYPE_IPV4_UC_E:
            ...
            lua_setfield(L, t+2, "dip");
            FO_NUMBER(&(key->key.ipv4Unicast), t+2, vrfId, GT_U32);
            lua_setfield(L, t+1, "ipv4Unicast");
            break;
        ...
        case CPSS_MAC_ENTRY_EXT_TYPE_IPV6_UC_ADDR_ENTRY_E:
            ...
            lua_setfield(L, t+2, "dip");
            FO_NUMBER(&(key->key.ipv6Unicast), t+2, vrfId, GT_U32);
            lua_setfield(L, t+1, "ipv6Unicast");
            break;
...
void prv_lua_to_CPSS_PM_PORT_ATTR_STC(
    lua_State *L,
...
*/
    ...
    F_NUMBER(val, -1, adaptRxTrainSupp, GT_BOOL);
    F_NUMBER(val, -1, edgeDetectSupported, GT_BOOL);
...
)
{
    ...
    FO_NUMBER(val, t, adaptRxTrainSupp,  GT_BOOL);
    FO_NUMBER(val, t, edgeDetectSupported, GT_BOOL);
...
/*
  ...
  CPSS_PM_AP_LANE_OVERRIDE_PARAM_STC      overrideLaneParams [CPSS_PM_MAX_LANE_OVERRIDE_SPEEDS][CPSS_PM_MAX_LANES];
*/
...
)
{
    ...
    lua_getfield(L, -1, "portParamsType");
    if (lua_istable(L, -1))
    {
        if (val->portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E)
        {
            F_STRUCT(&(val->portParamsType), -1, regPort, CPSS_PM_PORT_REG_PARAMS_STC);
        }
        else
        {
            F_STRUCT(&(val->portParamsType), -1, apPort, CPSS_PM_AP_PORT_PARAMS_STC);
        }
    }
    lua_pop(L, 1);
}
...
)
{
    ...
    FO_NUMBER(val, t, magic, GT_U32);
    {
        ...
        lua_newtable(L);
        t1 = lua_gettop(L);
            FO_STRUCT(&(val->portParamsType), t1, regPort, CPSS_PM_PORT_REG_PARAMS_STC);
            FO_STRUCT(&(val->portParamsType), t1, apPort, CPSS_PM_AP_PORT_PARAMS_STC);
