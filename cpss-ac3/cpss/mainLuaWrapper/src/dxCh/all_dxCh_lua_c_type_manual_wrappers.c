...
#define F_UNION_MEMBER_NUMBER(unionObject,Lindex,field,fieldtype) \
    F_NUMBER(&unionObject,Lindex,field,fieldtype)
    ...
    if (lua_istable(L, -1))
    {
        switch (val->type)
        {
            case CPSS_DXCH_IP_UC_ROUTE_ENTRY_E:
                F_UNION_MEMBER_STRUCT(val->entry, -1, regularEntry, CPSS_DXCH_IP_UC_ROUTE_ENTRY_FORMAT_STC);
                break;

            case CPSS_DXCH_IP_UC_ECMP_RPF_E:
                F_UNION_MEMBER_STRUCT(val->entry, -1, ecmpRpfCheck, CPSS_DXCH_IP_UC_ECMP_RPF_FORMAT_STC);
                break;
        }
    }
    lua_pop(L, 1);
...
)
{
    ...
    t1 = lua_gettop(L);
        ...
        {
            case CPSS_DXCH_IP_UC_ROUTE_ENTRY_E:
                FO_UNION_MEMBER_STRUCT(val->entry, t1, regularEntry, CPSS_DXCH_IP_UC_ROUTE_ENTRY_FORMAT_STC);
                break;

            case CPSS_DXCH_IP_UC_ECMP_RPF_E:
                FO_UNION_MEMBER_STRUCT(val->entry, t1, ecmpRpfCheck, CPSS_DXCH_IP_UC_ECMP_RPF_FORMAT_STC);
                break;
        }
    lua_setfield(L, t, "entry");
    /* ]]] done manually */
...
)
{
    ...
    F_UNION_MEMBER_STRUCT(val->defaultFcoeForwardingNextHopInfo, -1,
                       ipLttEntry, CPSS_DXCH_IP_LTT_ENTRY_STC);
}
...
)
{
    ...
    FO_UNION_MEMBER_STRUCT(val->defaultFcoeForwardingNextHopInfo, t,
                          ipLttEntry, CPSS_DXCH_IP_LTT_ENTRY_STC);
}
...
)
{
    ...
    prv_c_to_lua_CPSS_DXCH3_POLICER_METER_TB_PARAMS_UNT_srTcmParams(L, val);
    lua_setfield(L, t, "srTcmParams");

    prv_c_to_lua_CPSS_DXCH3_POLICER_METER_TB_PARAMS_UNT_trTcmParams(L, val);
    lua_setfield(L, t, "trTcmParams");

    prv_c_to_lua_CPSS_DXCH3_POLICER_METER_TB_PARAMS_UNT_envelope(L, val);
    lua_setfield(L, t, "envelope");
...
/*************************************************************************
* prv_lua_to_c_MV_HWS_MAN_TUNE_CTLE_CONFIG_OVERRIDE
*
* Description:
*       Convert to MV_HWS_MAN_TUNE_CTLE_CONFIG_OVERRIDE
*
* Parameters:
*       value at the top of stack
*
...
)
{
    ...
    lua_getfield(L, -1, "ctleParams");
    if (lua_istable(L, -1))
    {
        F_NUMBER(&(val->ctleParams), -1, dcGain, GT_U32);
        F_NUMBER(&(val->ctleParams), -1, lowFrequency, GT_U32);
        F_NUMBER(&(val->ctleParams), -1, highFrequency, GT_U32);
        F_NUMBER(&(val->ctleParams), -1, bandWidth, GT_U32);
        F_NUMBER(&(val->ctleParams), -1, loopBandwidth, GT_U32);
        F_NUMBER(&(val->ctleParams), -1, squelch, GT_U32);

    }
    lua_pop(L, 1);
    F_NUMBER(val, -1, fieldOverrideBmp, GT_U8);
}

/*************************************************************************
* prv_c_to_lua_MV_HWS_MAN_TUNE_CTLE_CONFIG_OVERRIDE
*
* Description:
*       Convert MV_HWS_MAN_TUNE_CTLE_CONFIG_OVERRIDE to lua
*
* Parameters:
*       value at the top of stack
*
...
)
{
    ...
    lua_newtable(L);
    t1 = lua_gettop(L);
    FO_NUMBER(&(val->ctleParams), t1, dcGain, GT_U32);
    FO_NUMBER(&(val->ctleParams), t1, lowFrequency, GT_U32);
    FO_NUMBER(&(val->ctleParams), t1, highFrequency, GT_U32);
    FO_NUMBER(&(val->ctleParams), t1, bandWidth, GT_U32);
    FO_NUMBER(&(val->ctleParams), t1, loopBandwidth, GT_U32);
    FO_NUMBER(&(val->ctleParams), t1, squelch, GT_U32);
    lua_setfield(L, t, "ctleParams");

    FO_NUMBER(val, t, fieldOverrideBmp, GT_U8);
}
...
SUPPORT_STC_TYPEDEF(CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_INGRESS_SWITCH_STC,CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_INGRESS_SWITCH_IPV4_STC);
SUPPORT_STC_TYPEDEF(CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_INGRESS_SWITCH_STC,CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_INGRESS_SWITCH_IPV6_STC);
SUPPORT_STC_TYPEDEF(CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_TRANSIT_SWITCH_STC,CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_TRANSIT_SWITCH_IPV4_STC);
SUPPORT_STC_TYPEDEF(CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_TRANSIT_SWITCH_STC,CPSS_DXCH_PHA_THREAD_INFO_TYPE_IOAM_TRANSIT_SWITCH_IPV6_STC);
...
)
{
    ...
    /* done manually [[[ */
    lua_getfield(L, -1, "phaThreadUnion");
    if (lua_istable(L, -1))
    {
        if(val->phaThreadType == CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_TYPE_NOT_USED_E)
        {
            F_UNION_MEMBER_NUMBER(val->phaThreadUnion, -1, notNeeded, GT_U32);
        }
        else
        {
            /*
            lua_getfield(L, -1, "epclIoamIngressSwitch");
            if (lua_istable(L, -1))
            {
                F_BOOL(&(val->phaThreadUnion.epclIoamIngressSwitch), -1, ioamIncludesBothE2EOptionAndTraceOption, GT_U32);
            }
            lua_pop(L, 1);
            */
            F_UNION_MEMBER_STRUCT(val->phaThreadUnion, -1, epclIoamIngressSwitch, CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IOAM_INGRESS_SWITCH_STC);
        }
    }
    lua_pop(L, 1);
    /* ]]] done manually */
