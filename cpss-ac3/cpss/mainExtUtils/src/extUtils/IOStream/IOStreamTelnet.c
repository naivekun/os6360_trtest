...
static int telnetStreamDestroy_PreAllocate(IOStreamPTR streamP)
{
    telnetStreamPTR stream = (telnetStreamPTR)streamP;
...
/* WNC_LUA_CLI_ENHANCE_IOP start */
IOStreamPTR IOStreamCreateTelnet_PreAllocate
(
    void
)
{
    telnetStreamPTR stream;
    IOStreamPTR socket_stream;
    unsigned char cmdBuf[3];
    static int socket_num = 10000;
    ...
    cpssOsMemSet(stream, 0, sizeof(*stream));
    stream->common.destroy = telnetStreamDestroy_PreAllocate;
    stream->common.read = telnetStreamRead;
    stream->common.readChar = telnetStreamReadChar;
    stream->common.readLine = IOStreamGenericReadLine;
    stream->common.write = telnetStreamWrite;
    ...
    stream->common.flags =
        ...
        IO_STREAM_FLAG_I_CANON |
        IO_STREAM_FLAG_I_ECHO;
    if ((socket_stream = IOStreamCreateSocket(socket_num++)))
        stream->transport = socket_stream;
