...
static int luaCLI_readLine(lua_State *L)
{
    ...
    line = (char*)memMgr->malloc(CLI_MAX_LINE_LEN);
    ...
    if (luaCLI_ReadLine(IOStream, L, prompt, flags, line, CLI_MAX_LINE_LEN) < 0)
    {
        memMgr->free(line);
        return 0;
    }
...
)
{
    ...
    while (IOStream->connected(IOStream))
    {
        if (state == STATE_CLI_E)
        {
            ...
            if (IS_CLI_SWITCH_MODE_TO_DX(line))
            {
            ...
            if (IS_CLI_SWITCH_MODE_TO_PX(line))
            {
...
)
{
    ...
    else
    {
        ...
        diff_sec = time_end_sec - time_start_sec;
        diff_nsec = time_end_ns - time_start_ns;
        cpssOsSprintf(buf, "Entering LuaCLI WNC IOP took %f msec\n", diff_sec * 1000 + (double)diff_nsec / 1000000);
        IOStream->writeLine(IOStream, buf);
