...
/* check that Exact Match group id is in valid range (APPLICABLE RANGES: Falcon: 0..3) */
#define PRV_TGF_EXACT_MATCH_GROUP_ID_DEV_CHECK_MAC(_devNum,_emGroup)                     \
...
typedef enum{
    ...
    /** EM client tunnel termination (APPLICABLE DEVICES: Falcon) */
    PRV_TGF_EXACT_MATCH_CLIENT_TTI_E,

    /** EM client ingress policy 0 (APPLICABLE DEVICES: Falcon) */
    PRV_TGF_EXACT_MATCH_CLIENT_IPCL_0_E,

    /** EM client ingress policy 1 (APPLICABLE DEVICES: Falcon) */
    PRV_TGF_EXACT_MATCH_CLIENT_IPCL_1_E,

   /** EM client egress policy (APPLICABLE DEVICES: Falcon) */
...
typedef struct{
    ...
    * bit 42 useVidx */
    ...
    /** @brief Used for building hash key and IPCL keys.
        ...
        but share the same EtherType (e.g., MPLS and MPLS-TP). 0 - Disabled:
        Use packet EtherType to select the UDB configuration table entry 1-
        Assign UDB configuration table index 9 2 - Assign UDB configuration
        table index 10 ..... 7 - Assign UDB configuration table
        index 15.
...
typedef struct{
    ...
    *  bit 16 PCL1 ovveride Config Index
    *  - IPCL only */
    GT_BOOL overwriteExpandedActionPCL1OverrideConfigIndex;

    /** @brief see CPSS_DXCH_PCL_ACTION_LOOKUP_CONFIG_STC
    *  bit 17 PCL2 ovveride Config Index
    *  - IPCL only */
    GT_BOOL overwriteExpandedActionPCL2OverrideConfigIndex;
...
/**
* @internal prvTgfExactMatchTtiProfileIdSet function
* @endinternal
*
...
*
*/
GT_STATUS  prvTgfExactMatchTtiProfileIdSet
(
    IN  GT_U8                               devNum,
...
/**
* @internal prvTgfExactMatchTtiProfileIdGet function
* @endinternal
*
...
*       CPSS_DXCH_TTI_KEY_MIM_E
*/
GT_STATUS prvTgfExactMatchTtiProfileIdGet
(
    IN  GT_U8                               devNum,
    IN  PRV_TGF_TTI_KEY_TYPE_ENT            keyType,
    IN  PRV_TGF_EXACT_MATCH_LOOKUP_ENT      ttiLookupNum,
