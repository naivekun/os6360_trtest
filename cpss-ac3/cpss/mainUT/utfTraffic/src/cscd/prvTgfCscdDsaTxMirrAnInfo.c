...
)
{
    ...
    */
    ...
    if(IS_AC3X_DEV_MAC(prvTgfDevNum))
    {
        ...
        rc = prvTgfCscdTxAnInitArrayByRandom(randArr, 2, 512, UTF_CPSS_PP_MAX_PORT_NUM_CNS(prvTgfDevNum)-1);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
                                    "prvTgfCscdTxAnInitArrayByRandom");
        prvTgfEport   = 512 + cpssOsRand() % 512 /* Max ePort number is 1023*/;
    }
    else
    {
        /* random ePorts > _2K to be different than any physical port number */
        rc = prvTgfCscdTxAnInitArrayByRandom(randArr, 2, _2K, UTF_CPSS_PP_MAX_PORT_NUM_CNS(prvTgfDevNum)-1);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
                                    "prvTgfCscdTxAnInitArrayByRandom");
        prvTgfEport   = _2K + cpssOsRand() % (UTF_CPSS_PP_MAX_PORT_NUM_CNS(prvTgfDevNum) - _2K);
    }

    /* random vlan id */
    prvTgfVlanId = 2 + cpssOsRand() % (_4K - 2);
...
{
    PRV_TGF_MIRROR_ANALYZER_INTERFACE_STC iface               = {{0}};
...
{
    CPSS_TRUNK_MEMBER_STC    trunkMember = {0};
...
{
    CPSS_INTERFACE_INFO_STC iface          = {0};
