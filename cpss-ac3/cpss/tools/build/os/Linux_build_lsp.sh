...
os_pre_build_int()
{
    ...
    if [ "x$DONT_CREATE_ROOTFS" != "xYES" ]; then
		...
		test -f $crfs_script || crfs_script=${DIST}/scripts/$LINUX_CRFS_SCRIPT_NAME

        I bash $crfs_script ${DIST}/rootfs ${TOOLKIT_PATH} ${LINUX_CRFS_ARCH_ARG} ${LINUX_DIST_PATH}
        if [ $? -ne 0 ]
        then
...
os_post_build_int()
{
	...
	if [ "$DONT_BUILD_IMAGES" != "YES" ]; then
		cr_script=$tool_build/toolkit/${TOOLKIT}/${CR_JFFS2_FILE_NAME}
		test -f $cr_script || cr_script=${DIST}/scripts/${CR_JFFS2_FILE_NAME}
