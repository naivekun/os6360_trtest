...
export LSP_KERNEL_TYPE=${LSP_KERNEL_TYPE:-uImage}
...
{
    busybox_versions_supported="1.25.0 1.01 1.2.1"
    bbversion=""
    for ver in $busybox_versions_supported
    do
        if [ -e busybox-$ver.tar.bz2 -o -e busybox-$ver.tar.gz ]
        then
            bbversion=$ver
            break
        fi
    ...
    then
        ...
        trace_message -e "\tBusybox $bbversion tarball found"
    fi

    test -d ${DIST}/rootfs || return 0
    T mkdir ${DIST}/rootfs
    if [ $? -ne 0 ]
    then
...
os_pre_build_mk_uImage()
{
    ...
    if [        "$LSP_KERNEL_TYPE" = "zImage" \
        ...
        for index in ${!LSP_KERNEL_UDTB_NAME[*]}; do
            ...
            rm -f $tmpfile
            cat ${objs}/arch/arm/boot/zImage ${objs}/arch/arm/boot/dts/$dtb > $tmpfile
...
os_pre_build()
{
    ...
    then
        info_message " >>> os_pre_build : LINUX_BUILD_KERNEL - $LINUX_BUILD_KERNEL : no build linux kernel <<<"
        return 0
    fi
    ...
    if [ -d $DIST/rootfs ]; then
        rm -f $DIST/rootfs/usr/lib/*.so
        rm -f $DIST/rootfs/usr/bin/appDemo*
        rm -f $DIST/rootfs/usr/bin/RxTxProcess
        rm -f $DIST/rootfs/usr/bin/fdbLearning
        rm -f $DIST/rootfs/usr/bin/luaCLI
        rm -f $DIST/rootfs/3240_0400_6270.bin
        rm -f $DIST/rootfs/88X33xx-Z2-FW.hdr
        rm -f $DIST/rootfs/x3310fw_0_2_8_0_8850.hdr
	rm -f $DIST/rootfs/e21x0fw_8_0_0_36_8923.hdr
        rm -f $DIST/rootfs/88X32xx-FW.hdr
        rm -f $DIST/rootfs/flashdownloadslave.bin
        find $DIST/rootfs/lib/modules -type f -a -name '*.ko' -exec rm -f '{}' \;
    fi
...
os_post_build()
{
    if [ "${LINUX_BUILD_KERNEL}" != YES ]
    then
       info_message " >>> os_post_build : LINUX_BUILD_KERNEL - $LINUX_BUILD_KERNEL : no build linux kernel <<<"
       if [ "${CPSS_LIB_ONLY}" = "YES" ]
       then
            info_message  " >>> os_post_build : CPSS_LIB_ONLY - $CPSS_LIB_ONLY : copy cpss lib to $IMAGE_PATH <<<"
            I cp ${COMPILATION_ROOT}/cpssDrvLib.a  ${IMAGE_PATH}
       fi
       return 0
    fi
    ...
    for drv_dir in $COMPILATION_ROOT/_drivers/*
    do
        if [ -d $drv_dir ]; then
            $MAKE -C $drv_dir all
        fi
