...
elif [ "$FORCE_GM_ALDRIN2_USE" = "EXISTS" ];then
   CPU_DIR=VC10_GM_ALDRIN2 
elif [ "$FORCE_GM_FALCON_USE" = "EXISTS" ];then
   CPU_DIR=VC10_GM_FALCON 
else
   CPU_DIR=VC10
fi
...
if [ -n "$ROL_WINE_ENV" ]; then
	export CC=${VC_CROSS_COMPILE}cl
	export CXX=${VC_CROSS_COMPILE}cl
	export AR=${VC_CROSS_COMPILE}lib
	export LD=${VC_CROSS_COMPILE}link
	export CPP=${VC_CROSS_COMPILE}cl
fi
