...
}' $ROOT_FS/../rootfs_deviceMap >> $ROOT_FS/dev/MKDEV.sh
...
# 
# mkfs.jffs2 --eraseblock=${JFFS2_ERASE_BLOCK_SIZE:-16}KiB ${endianess} -p -n \
# 	-d ${ROOT_FS} -D ${ROOT_FS}/../rootfs_deviceMap \
# 	-o ${TEMP_IMAGE}
# 
...
# create ubi image
mkfs.ubifs -r ${ROOT_FS} -D ${ROOT_FS}/../rootfs_deviceMap \
	-m 4096 -e 253952 -c 4096 -R 24 -o ${OUTPUT_DIR}/temp_ubi_rootfs.img -v
ubinize -o ${OUTPUT_DIR}/temp_ubifs_rootfs.img -m 4096  -p 256KiB -s 4096 ${UBI_CFG} -v

# concat kernel zImage/uImage, DTB and ubi image
bzip2 -c ${KERNEL} > ${IMAGE}
if [ "$DTB" != "NONE" ]; then
  bzip2 -c ${DTB} >> ${IMAGE}
fi
...
# create ubi image
mkfs.ubifs -r ${ROOT_FS} -D ${ROOT_FS}/../rootfs_deviceMap \
	-m 4096 -e 516096 -c 4096 -R 24 -o ${OUTPUT_DIR}/temp_ubi_rootfs.img -v
